...
## SSキャプチャモード
余白のないきれいなSSをキャプチャすることが出来ます。
キャプチャしたSSは「ScreenShots」フォルダに保存されます。
...
追跡が有効だと、艦これを表示しているブラウザの位置を動かしても問題なくキャプチャが出来ます
 ...
 「取得する」ボタンを押すと艦これの画面が検索され、座標を取得します<br>
  数回試して失敗する場合は手動取得をお試しください
  * ##### 手動座標取得
  自動座標取得にチェックが入っていないと手動取得になります<br>
 「取得する」ボタンを押し、艦これの画面を完全に覆うようにドラッグします
 * ##### 追跡について
追跡は、__ブラウザ内でスクロールを行ったりブラウザのサイズ/倍率変更が行われると自動的に解除__ されます<br>
環境によって動作が変わり、自動的に利用可能な最良のものが選ばれます
    * 追跡中S
      * 追跡が有効で、かつ艦これの画面上に何かが重なっても映り込むことなくキャプチャできます<br>
	※ブラウザの相性により毎回同じ画像になるなどの不具合が生じる可能性があります<br>
...
* #### 動画キャプチャ
一覧めいかー改二をダウンロードしたての状態ではまだ使うことが出来ません<br>
動画キャプチャ機能を使うには、<br>
[お気に入りの動画を携帯で見よう](http://blog.k-tai-douga.com/)などからffmpegをダウンロードし、「ffmpeg.exe」をbinフォルダに入れるか[オプション→動画キャプチャ設定]から選択してください<br>
ffmpegが読み込まれると「使用するオーディオデバイス」が表示されます(選択必須)<br>
動画キャプチャ中はホットキーによるキャプチャ以外の操作を行う事はできません<br>
詳細は下の「動画キャプチャについて」をご覧ください<br>
※キャプチャされた動画は一覧めいかー改二ではアップロードすることが出来ません<br>
Twitterにアップロードする場合はWebクライアントなどからご利用下さい<br>
 __※ffmpegの選択で艦これ一覧めいかー改二を絶対に指定しない__<br>

## 一覧作成モード(まとめ画像の作成)
各モードに対応した画面でホットキーによるキャプチャか、スクリーンショットのドラッグ&ドロップで画像を並べられます<br>
セルに置いた画像はドラッグで移動でき、画面外への移動で画像を削除でき、<br>
右クリックで範囲をドラッグすると複数の画像を一度に移動することができます<br>
 ...
 * 「自動追加の方法」はホットキーやメインウィンドウへのドラッグアンドドロップで自動追加した際にどう並べていくかを指定します
 * 「キャプチャ範囲」は一覧を作るときにどこまで表示するかを指定します
 * 「艦隊区切り線」にチェックを入れると「自動追加の方法」に応じた区切り線が表示、作成時には追加されます
 * 艦隊名はチェックが入っているものが作成時に追加されます
 __全角文字約7文字ではみ出す__ のでそれ以上の文字を表示させたい場合はフォントサイズの調節(iniファイルの編集)をしてください
 * 「西村艦隊(7人)に対応」
 「7人編成」へ対応するオプションです。
  7人の編成＋決戦支援の2艦隊分に対応しています。
...
Atomを搭載したタブレットPCでは撮影された動画が極端にカクついたり、音声が入らない場合があります<br>
デスクトップやノートPCの場合、2013年以降に発売された極端な廉価モデルで無ければキャプチャ可能と思われます<br>
参考に、800x480pxの艦これの画面を対象としたキャプチャ時のCPU負荷はCore i7 3770kで5%未満、i5 5250Uで10～15%程度です<br>
* #### 音声について
音声を録音する場合、PCのステレオミキサーと呼ばれる機能をを有効にするか、外部ソフトを導入する必要があります<br>
...
ステレオミキサーを利用してうまく音声が入らない場合は非搭載の場合の手順をお試し下さい。<br><br>
 * ステレオミキサー非搭載の場合、「on screen capture recorder to video free」「アマミキ！」などを導入すると音声をキャプチャ出来ます。
   * __「on screen capture recorder to video free」__ の場合 __(おすすめ)__<br>
...
Java何たらと表示が出てインストールが上手くいかない場合はJavaのインストールが必要です<br>
