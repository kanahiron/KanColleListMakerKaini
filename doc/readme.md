...
動画キャプチャ機能を使うにはffmpeg.exeと呼ばれる外部ソフトが必要です<br>
  ...
  * ffmpegが読み込まれると「使用するオーディオデバイス」が表示されますので、選択してください<br><br>
  ...
  ここまで設定を行うと、SSキャプチャモードで「動画キャプチャ開始」ボタンが押せるようになります<br>
  ※動画キャプチャ中はホットキーによるキャプチャ以外の操作を行う事はできません<br>
__詳細は下の「動画キャプチャについて」をご覧ください__<br><br>
 ...
 * ステレオミキサー非搭載の場合、「on screen capture recorder to video free」や「アマミキ！」などを導入すると音声をキャプチャ出来ます。
...
インストールが終わったら、一覧めいかー改二の[オプション→動画キャプチャ]を開くと「使用するオーディオデバイス」に「virtual-audio-capturer」というのが出ますので、それを選択し完了です<br><br>
    * 「アマミキ！」の場合<br>
